...
| ST-05 | Failure during `RECOVERING` | Immediately return to `DOWN` with `recovery-failed`, set success to zero and failure to one, retain the active incident, increment occurrence once, and emit one suppressed `recovery-failed` occurrence event/context. |
