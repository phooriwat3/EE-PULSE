...
Status: Phase 2A timezone-preference implementation plus Phase 2B1A dashboard-summary runtime implementation (2026-09-09)
Scope: Phase 2A implements the timezone-preference contract, persistence foundation, API slice, authorization, and generated OpenAPI. Phase 2B1A implements the frozen `GET /api/v1/dashboard/summary` runtime slice. That route is intentionally excluded from endpoint description and therefore remains absent from the generated OpenAPI artifact until the later OpenAPI checkpoint. The Phase 2B1 device-status runtime endpoint remains pending; OpenAPI is otherwise unchanged.

## Phase 2B1 frozen runtime contract

`GET /api/v1/dashboard/summary` is implemented as the Phase 2B1A runtime slice. `GET /api/v1/devices/{id}/status` remains pending. Metrics, a status-enriched device list, history/timeline, incident reads/actions/comments, audit reads, SignalR runtime, frontend behavior, WP-08 notification fan-out, and WP-09 reporting/retention remain excluded. In particular, `includeStatus` is not added to the existing device-list response.
...
GET conditional behavior accepts `If-None-Match` entity-tag lists across multiple header lines. Wildcard is valid only as the sole member. Supplied weak and strong tags use weak comparison for GET. Malformed input is `400` code `invalid-if-none-match`; a match is bodyless `304` with the current strong ETag, `Cache-Control: private, max-age=0, must-revalidate`, and `X-Correlation-ID`; a non-match is canonical `200`. `If-Match` is unsupported for these reads.

For `GET /api/v1/dashboard/summary`, invalid caller filters remain `400`. A database/provider read failure or server-side response validation/canonicalization failure is instead a sanitized `503 Service Unavailable` with code `dashboard-summary-unavailable` and the server-generated `X-Correlation-ID`; it never exposes internal exception, SQL, provider, entity, or stack details. Caller cancellation is preserved and is not translated to `503`.

## 1. Boundaries and compatibility

The existing v1 surface remains additive and compatibility-owned. Phase 2A adds the verified `GET` and `PUT /api/v1/users/me/timezone-preference` operations to the generated `docs/api/openapi-v1.json`. Phase 2B1A additionally implements `GET /api/v1/dashboard/summary`, but intentionally excludes it from endpoint description so it remains absent from that generated artifact until the later OpenAPI checkpoint.
