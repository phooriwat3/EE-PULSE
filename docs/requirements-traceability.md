...
| FR-09 Authentication/authorization | WP-02, WP-03, WP-07, WP-11 | In progress | Verified inventory policies, development-only attributable header identity/UI, production API and Web fail-closed behavior, production-bundle exclusion of synthetic identity, audit attribution, anonymous 401, forbidden/read-only UI states, and OpenAPI security metadata. Production OIDC and full role matrix remain. |
