...
public sealed class AgentApiTests
{
    ...
    public async Task EnrollmentRequiresPublishedPolicyAndExpiredOrRevokedTokensNeverMutate()
    {
        ...
        await using var factory = new WebApplicationFactory<Program>().WithIncidentCursorKeyRing(IncidentCursorKeyRingTestHost.CreateRing()).WithWebHostBuilder(builder => builder.UseSetting("ConnectionStrings:Postgres", postgres.ConnectionString)); using var client = factory.CreateClient(); var group = await CreateGroup(client, ct);
    ...
    {
        var ct = TestContext.Current.CancellationToken; await using var postgres = await PostgresTestDatabase.StartAsync(ct); await using var factory = new WebApplicationFactory<Program>().WithIncidentCursorKeyRing(IncidentCursorKeyRingTestHost.CreateRing()).WithWebHostBuilder(builder => builder.UseSetting("ConnectionStrings:Postgres", postgres.ConnectionString)); using var client = factory.CreateClient(); var agentId = Guid.NewGuid();
    ...
    {
        var ct = TestContext.Current.CancellationToken; await using var postgres = await PostgresTestDatabase.StartAsync(ct); await using var factory = new WebApplicationFactory<Program>().WithIncidentCursorKeyRing(IncidentCursorKeyRingTestHost.CreateRing()).WithWebHostBuilder(builder => builder.UseSetting("ConnectionStrings:Postgres", postgres.ConnectionString)); using var client = factory.CreateClient(); var group = await CreateGroup(client, ct); _ = await SendAdmin<AgentNetworkPolicyResponse>(client, HttpMethod.Put, $"/api/v1/agent-groups/{group.Id}/allowed-networks", new UpdateAgentGroupAllowedNetworksRequest(1, ["192.0.2.0/24"], group.RowVersion), ct);
    ...
    {
        var ct = TestContext.Current.CancellationToken; await using var postgres = await PostgresTestDatabase.StartAsync(ct);
        await using var factory = new WebApplicationFactory<Program>().WithIncidentCursorKeyRing(IncidentCursorKeyRingTestHost.CreateRing()).WithWebHostBuilder(builder => builder.UseSetting("ConnectionStrings:Postgres", postgres.ConnectionString)); using var client = factory.CreateClient();
    ...
    {
        var ct = TestContext.Current.CancellationToken; await using var postgres = await PostgresTestDatabase.StartAsync(ct);
        await using var factory = new WebApplicationFactory<Program>().WithIncidentCursorKeyRing(IncidentCursorKeyRingTestHost.CreateRing()).WithWebHostBuilder(builder => { builder.UseSetting("ConnectionStrings:Postgres", postgres.ConnectionString); builder.ConfigureTestServices(services => services.AddSingleton(new AgentRateLimitDefaults(100, 20, 60))); }); using var client = factory.CreateClient(); var group = await CreateGroup(client, ct); _ = await SendAdmin<AgentNetworkPolicyResponse>(client, HttpMethod.Put, $"/api/v1/agent-groups/{group.Id}/allowed-networks", new UpdateAgentGroupAllowedNetworksRequest(1, ["192.0.2.0/24"], group.RowVersion), ct); var issued = await Issue(client, group.Id, ct);
    ...
    {
        var ct = TestContext.Current.CancellationToken; await using var postgres = await PostgresTestDatabase.StartAsync(ct);
        await using var factory = new WebApplicationFactory<Program>().WithIncidentCursorKeyRing(IncidentCursorKeyRingTestHost.CreateRing()).WithWebHostBuilder(b => b.UseSetting("ConnectionStrings:Postgres", postgres.ConnectionString)); using var client = factory.CreateClient();
    ...
    public async Task St10bH2HeartbeatWithoutOwnedProjectionPersistsReceiptWithoutCause()
    {
        ...
        var expectedLastReportedAt = NormalizeMicroseconds(request.SentAt);
        Assert.Equal((fixture.AgentId, request.MachineName, "1.2.3", AgentSelfHealth.Healthy, AgentStatus.Online, 0L, 20, expectedLastReportedAt, response.ReceivedAt, false),
        ...
        var expectedReceiptJson = await NormalizePostgresJsonbAsync(postgres.ConnectionString, System.Text.Json.JsonSerializer.Serialize(response, AgentJson), ct);
        Assert.Equal(expectedReceiptJson, receipt.ResponseJson);
        Assert.Equal(response, System.Text.Json.JsonSerializer.Deserialize<AgentHeartbeatResponse>(receipt.ResponseJson, AgentJson));
    ...
    {
        var ct = TestContext.Current.CancellationToken; await using var postgres = await PostgresTestDatabase.StartAsync(ct); await using var factory = new WebApplicationFactory<Program>().WithIncidentCursorKeyRing(IncidentCursorKeyRingTestHost.CreateRing()).WithWebHostBuilder(b => b.UseSetting("ConnectionStrings:Postgres", postgres.ConnectionString)); using var client = factory.CreateClient();
    ...
    {
        var ct = TestContext.Current.CancellationToken; await using var postgres = await PostgresTestDatabase.StartAsync(ct); await using var factory = new WebApplicationFactory<Program>().WithIncidentCursorKeyRing(IncidentCursorKeyRingTestHost.CreateRing()).WithWebHostBuilder(b => b.UseSetting("ConnectionStrings:Postgres", postgres.ConnectionString)); using var client = factory.CreateClient(); var fixture = await CreateSt10bHeartbeatFixtureAsync(factory, client, true, ct); var baseline = await ReadSt10bHeartbeatSnapshotAsync(factory, fixture.AgentId, ct); var request = Heartbeat(Guid.Parse("90000000-0000-0000-0000-000000000031")); var resource = await InstallSt10bFailureTriggerAsync(postgres.ConnectionString, fixture.AgentId, ct); Exception? primary = null;
        try
        {
            using var failedRequest = AgentRequest(HttpMethod.Post, $"/api/v1/agents/{fixture.AgentId}/heartbeat", fixture.Credential, AgentContent(request)); var failed = await client.SendAsync(failedRequest, ct); Assert.Equal(HttpStatusCode.InternalServerError, failed.StatusCode); Assert.Equal("application/problem+json", failed.Content.Headers.ContentType?.MediaType); var problem = await failed.Content.ReadFromJsonAsync<ProblemDetails>(ct); Assert.NotNull(problem); Assert.Equal(500, problem.Status); Assert.Equal("An error occurred while processing your request.", problem.Title); Assert.Equal("https://tools.ietf.org/html/rfc9110#section-15.6.1", problem.Type);
        }
    ...
    public async Task T4A2H2AcquiresReceivingAgentBeforeRequestingCanonicalProbeLock()
    {
        ...
        await using var factory = new WebApplicationFactory<Program>().WithIncidentCursorKeyRing(IncidentCursorKeyRingTestHost.CreateRing()).WithWebHostBuilder(builder => builder.UseSetting("ConnectionStrings:Postgres", h2ConnectionString));
        using var client = factory.CreateClient();
        ...
        St10bFullAgentSnapshot agentBefore;
        await using (var scope = factory.Services.CreateAsyncScope())
        {
            var db = scope.ServiceProvider.GetRequiredService<EePulseDbContext>();
            agentBefore = await db.Agents.AsNoTracking().Where(x => x.Id == fixture.AgentId)
                .Select(x => new St10bFullAgentSnapshot(x.Id, x.AgentGroupId, x.ClientInstanceId, x.Name, x.MachineName, x.AgentVersion, x.SelfHealth, x.Status, x.QueueDepth, x.LastHeartbeatAt, x.LastReportedAt, x.HeartbeatIntervalSeconds, x.DesiredConfigurationVersion, x.LastAppliedConfigurationVersion, x.LastConfigurationAcknowledgedAt, x.ClockSkewSuspected, x.CredentialExpiresAt, x.CreatedAt, x.RevokedAt, x.RevocationReason, x.RowVersion))
                .SingleAsync(ct);
        ...
        HttpRequestMessage? heartbeatRequest = null; Task<HttpResponseMessage>? heartbeatTask = null; HttpResponseMessage? heartbeatResponse = null; Task? waitC = null; NpgsqlCommand? waitCCommand = null; CancellationTokenSource? waitCCancellation = null; int? pidB = null; Exception? primary = null; ExceptionDispatchInfo? primaryDispatch = null; Exception? cleanupToThrow = null; var releasedA = false; var releasedC = false;
        try
        {
        ...
        var expectedReceiptJson = await NormalizePostgresJsonbAsync(postgres.ConnectionString, System.Text.Json.JsonSerializer.Serialize(response, AgentJson), ct);
        Assert.Equal((fixture.AgentId, heartbeatId, response.ReceivedAt, expectedReceiptJson), (receipt.AgentId, receipt.HeartbeatId, receipt.ReceivedAt, receipt.ResponseJson));
        Assert.Equal(response, System.Text.Json.JsonSerializer.Deserialize<AgentHeartbeatResponse>(receipt.ResponseJson, AgentJson));
        var expectedLastReportedAt = NormalizeMicroseconds(requestBody.SentAt);
        var expectedAgentAfter = agentBefore with
        {
            MachineName = requestBody.MachineName,
            AgentVersion = requestBody.AgentVersion,
            SelfHealth = AgentSelfHealth.Healthy,
            Status = AgentStatus.Online,
            QueueDepth = requestBody.QueueDepth,
            LastHeartbeatAt = response.ReceivedAt,
            LastReportedAt = expectedLastReportedAt,
            ClockSkewSuspected = (response.ReceivedAt - expectedLastReportedAt).Duration() > TimeSpan.FromMinutes(5),
            RowVersion = agentBefore.RowVersion + 1
        };
        await using (var agentVerifyScope = factory.Services.CreateAsyncScope())
        {
            var agentVerifyDb = agentVerifyScope.ServiceProvider.GetRequiredService<EePulseDbContext>();
            var persistedAgent = await agentVerifyDb.Agents.AsNoTracking().Where(x => x.Id == fixture.AgentId)
                .Select(x => new St10bFullAgentSnapshot(x.Id, x.AgentGroupId, x.ClientInstanceId, x.Name, x.MachineName, x.AgentVersion, x.SelfHealth, x.Status, x.QueueDepth, x.LastHeartbeatAt, x.LastReportedAt, x.HeartbeatIntervalSeconds, x.DesiredConfigurationVersion, x.LastAppliedConfigurationVersion, x.LastConfigurationAcknowledgedAt, x.ClockSkewSuspected, x.CredentialExpiresAt, x.CreatedAt, x.RevokedAt, x.RevocationReason, x.RowVersion))
                .SingleAsync(ct);
            Assert.Equal(expectedAgentAfter, persistedAgent);
        }
    ...
    public async Task T4B1H1FirstAppliesThenWaitingH2CreatesSuccessorGeneration()
    {
        ...
        await using var factory = new WebApplicationFactory<Program>().WithIncidentCursorKeyRing(IncidentCursorKeyRingTestHost.CreateRing()).WithWebHostBuilder(builder => builder.UseSetting("ConnectionStrings:Postgres", h2ConnectionString)); using var client = factory.CreateClient();
        ...
        await using (var setupScope = factory.Services.CreateAsyncScope())
        {
            ...
            agent.Heartbeat(agent.AgentVersion, agent.MachineName, agent.QueueDepth, AgentSelfHealth.Healthy, agent.DesiredConfigurationVersion, oldHeartbeat, oldHeartbeat);
            await EstablishSt10bAppliedConfigurationAsync(db, fixture.AgentId, 100, Guid.Parse("93000000-0000-0000-0000-000000000003"), sourceEventAt.AddSeconds(-1), ct);
            ...
            var outcome = await new ProbeResultStatusProcessor(db, new FixedClock(sourceEventAt)).ProcessNextAsync(initialSource.ProbeId, ct);
            Assert.Equal((ProbeResultStatusProcessorOutcomeKind.Processed, fixture.AgentId, sourceResultId, ProbeResultProcessingDispositionKind.StateDriving), (outcome.Kind, outcome.AgentId, outcome.ResultId, outcome.Disposition));
        }
        ...
        await using (var preScope = factory.Services.CreateAsyncScope())
        {
            ...
            Assert.Equal((initialSource.ProbeId, ProbeStatus.Up, ProbeStatus.Up, fixture.AgentId, sourceResultId, sourceEventAt, sourceEventAt, 0, 1, 1L, (Guid?)null), (preRaceProjection.ProbeId, preRaceProjection.UnderlyingStatus, preRaceProjection.VisibleStatus, preRaceProjection.WatermarkAgentId, preRaceProjection.WatermarkResultId, preRaceProjection.WatermarkEventAt, preRaceProjection.LastFreshEventAt, preRaceProjection.ConsecutiveFailureCount, preRaceProjection.ConsecutiveSuccessCount, preRaceProjection.StateVersion, preRaceProjection.OpenIncidentId));
            ...
            var expectedResultDispositions = new[]
            {
                new St10bResultDispositionSnapshot(fixture.AgentId, initialSource.ResultId, initialSource.ProbeId,
                    initialSource.EventAt, ProbeResultProcessingDispositionKind.StateDriving, "state-driving",
                    fixture.PolicyId, 1, initialSource.EventAt),
                new St10bResultDispositionSnapshot(fixture.AgentId, sourceResultId, initialSource.ProbeId,
                    sourceEventAt, ProbeResultProcessingDispositionKind.StateDriving, "state-driving",
                    fixture.PolicyId, 1, sourceEventAt),
            };
            Assert.True(expectedResultDispositions.SequenceEqual(artifactsBefore.ResultDispositions));
            Assert.Empty(artifactsBefore.ResultTransitions); Assert.Empty(artifactsBefore.Incidents); Assert.Empty(artifactsBefore.Events); Assert.Empty(artifactsBefore.Contexts);
        }
        ...
        await using var observer = new NpgsqlConnection(postgres.ConnectionString); await observer.OpenAsync(ct); var observerPid = await GetBackendPidAsync(observer, ct);
        var h1Db = new EePulseDbContext(new DbContextOptionsBuilder<EePulseDbContext>().UseNpgsql(h1ConnectionString).Options); HttpRequestMessage? h2Request = null; Task<ProbeHeartbeatExpiryProcessorOutcome>? h1Task = null; Task<HttpResponseMessage>? h2Task = null; HttpResponseMessage? h2Response = null; string? h2Payload = null; int? pidB = null; int? pidC = null; Exception? primary = null; ExceptionDispatchInfo? primaryDispatch = null; Exception? cleanupToThrow = null; var releasedA = false;
        try
        {
            ...
            await txA.RollbackAsync(ct); releasedA = true;
            using var bounded = CancellationTokenSource.CreateLinkedTokenSource(ct); bounded.CancelAfter(TimeSpan.FromSeconds(10)); var h1 = await h1Task.WaitAsync(bounded.Token); h2Response = await h2Task.WaitAsync(bounded.Token); h2Payload = await h2Response.Content.ReadAsStringAsync(bounded.Token);
        ...
        finally
        {
            ...
            if (cleanupFailures.Count == 0) { } else if (primary is not null) for (var index = 0; index < cleanupFailures.Count; index++) primary.Data[$"T4B1CleanupFailure{index + 1}"] = cleanupFailures[index]; else cleanupToThrow = cleanupFailures.Count == 1 ? cleanupFailures[0] : new AggregateException(cleanupFailures);
        }
        primaryDispatch?.Throw();
        if (cleanupToThrow is not null) ExceptionDispatchInfo.Capture(cleanupToThrow).Throw();
        ...
        var canonicalResponseJson = System.Text.Json.JsonSerializer.Serialize(response, AgentJson);
        var postgresJsonbResponseJson = await NormalizePostgresJsonbAsync(postgres.ConnectionString, canonicalResponseJson, ct);
    ...
    public async Task T4B2H2FirstCreatesSuccessorThenWaitingH1RecordsHeartbeatAdvancedNoOp()
    {
        ...
        var factory = new WebApplicationFactory<Program>().WithIncidentCursorKeyRing(IncidentCursorKeyRingTestHost.CreateRing()).WithWebHostBuilder(b => b.UseSetting("ConnectionStrings:Postgres", h2Connection)); var client = factory.CreateClient(); var fixture = await CreateSt10bHeartbeatFixtureAsync(factory, client, true, ct, eligibleProjectionCount: 1); var source = Assert.Single(fixture.Probes, x => x.HasProjection);
        ...
        await using (var setup = factory.Services.CreateAsyncScope()) { var db = setup.ServiceProvider.GetRequiredService<EePulseDbContext>(); var agent = await db.Agents.SingleAsync(x => x.Id == fixture.AgentId, ct); agent.Heartbeat(agent.AgentVersion, agent.MachineName, agent.QueueDepth, AgentSelfHealth.Healthy, agent.DesiredConfigurationVersion, oldHeartbeat, oldHeartbeat); await EstablishSt10bAppliedConfigurationAsync(db, fixture.AgentId, 100, Guid.Parse("94000000-0000-0000-0000-000000000003"), sourceEventAt.AddSeconds(-1), ct); db.Add(new ProbeResultLedgerEntry(fixture.AgentId, sourceResultId, source.ProbeId, 100, sourceEventAt.AddSeconds(-1), sourceEventAt, 1, 1, 0m, 1m, 1m, 1m, null, new byte[32], sourceEventAt)); await db.SaveChangesAsync(ct); var outcome = await new ProbeResultStatusProcessor(db, new FixedClock(sourceEventAt)).ProcessNextAsync(source.ProbeId, ct); Assert.Equal((ProbeResultStatusProcessorOutcomeKind.Processed, fixture.AgentId, sourceResultId, ProbeResultProcessingDispositionKind.StateDriving), (outcome.Kind, outcome.AgentId, outcome.ResultId, outcome.Disposition)); }
        ...
        var expectedProjectionBefore = new St10bProjectionSnapshot(source.ProbeId, ProbeStatus.Up, ProbeStatus.Up, 0, 1, 1L, fixture.AgentId, sourceResultId, sourceEventAt, sourceEventAt, null);
    ...
    {
        var agent = await db.Agents.SingleAsync(x => x.Id == agentId, cancellationToken);
        agent.SetDesiredConfiguration(configurationVersion);
        agent.AcknowledgeApplied(configurationVersion, effectiveAt);
        var acknowledgement = new AgentConfigurationAcknowledgement(
            acknowledgementId,
            agentId,
            configurationVersion,
            AgentAcknowledgementStatus.Applied,
            effectiveAt,
            effectiveAt,
            effectiveAt,
            null,
            configurationVersion,
            configurationVersion);
        var boundary = new AgentConfigurationEffectiveBoundary(
            agentId,
            configurationVersion,
            acknowledgementId,
            AgentAcknowledgementStatus.Applied,
            effectiveAt);
        db.AddRange(acknowledgement, boundary);
        await db.SaveChangesAsync(cancellationToken);

        var persistedAgent = await db.Agents.AsNoTracking().SingleAsync(x => x.Id == agentId, cancellationToken);
        var persistedAcknowledgement = await db.AgentConfigurationAcknowledgements.AsNoTracking()
            .SingleAsync(x => x.AgentId == agentId && x.Id == acknowledgementId, cancellationToken);
        var persistedBoundary = await db.AgentConfigurationEffectiveBoundaries.AsNoTracking()
            .SingleAsync(x => x.AgentId == agentId && x.ConfigurationVersion == configurationVersion, cancellationToken);

        Assert.Equal((agentId, configurationVersion, configurationVersion, effectiveAt),
            (persistedAgent.Id, persistedAgent.DesiredConfigurationVersion, persistedAgent.LastAppliedConfigurationVersion, persistedAgent.LastConfigurationAcknowledgedAt));
        Assert.Equal((acknowledgementId, agentId, configurationVersion, AgentAcknowledgementStatus.Applied, (DateTimeOffset?)effectiveAt, effectiveAt, effectiveAt, (string?)null, configurationVersion, configurationVersion),
            (persistedAcknowledgement.Id, persistedAcknowledgement.AgentId, persistedAcknowledgement.ConfigurationVersion, persistedAcknowledgement.Status, persistedAcknowledgement.AppliedAt, persistedAcknowledgement.SentAt, persistedAcknowledgement.ReceivedAt, persistedAcknowledgement.ErrorCode, persistedAcknowledgement.CentralEffectiveConfigurationVersion, persistedAcknowledgement.DesiredConfigurationVersion));
        Assert.Equal((agentId, configurationVersion, acknowledgementId, AgentAcknowledgementStatus.Applied, effectiveAt),
            (persistedBoundary.AgentId, persistedBoundary.ConfigurationVersion, persistedBoundary.SourceAcknowledgementId, persistedBoundary.SourceAcknowledgementStatus, persistedBoundary.AppliedAcknowledgementReceivedAt));
    }
    ...
    private static async Task<T4B2BackendIdentity> CaptureT4B2BackendIdentityAsync(NpgsqlConnection observer, string applicationName, Task task, CancellationToken cancellationToken)
    {
        ...
        try { while (true) { attempts++; await using var command = new NpgsqlCommand("SELECT pid, application_name, backend_start::text, datname FROM pg_stat_activity WHERE application_name = @applicationName AND state <> 'idle' ORDER BY pid", observer); command.Parameters.AddWithValue("applicationName", applicationName); var identities = new List<T4B2BackendIdentity>(); var observations = new List<string>(); await using var reader = await command.ExecuteReaderAsync(timeout.Token); while (await reader.ReadAsync(timeout.Token)) { var pid = reader.GetInt32(0); var observedApplication = reader.GetString(1); var backendStart = reader.GetString(2); var database = reader.IsDBNull(3) ? null : reader.GetString(3); observations.Add($"pid={pid},application={observedApplication},backend_start={backendStart},database={(database ?? "<null>")}"); if (database is not null) identities.Add(new T4B2BackendIdentity(pid, observedApplication, backendStart, database)); } last = observations.Count == 0 ? "<none>" : string.Join(" | ", observations); if (observations.Count == 1 && identities.Count == 1) return identities[0]; if (task.IsCompleted) { await task; throw new Xunit.Sdk.XunitException($"T4B2 operation completed before one complete backend identity was observable. ApplicationName={applicationName}; backends={last}; attempts={attempts}; elapsed={DateTimeOffset.UtcNow - started}."); } await Task.Delay(TimeSpan.FromMilliseconds(20), timeout.Token); } }
    ...
    private sealed class T4B2DeferredCleanupDiagnosticHolder(string operationId)
    {
        ...
        public T4B2DeferredCleanupOutcome[] Snapshot() { lock (gate) return outcomes.ToArray(); }
    }
    ...
    private static async Task<string> NormalizePostgresJsonbAsync(string connectionString, string json, CancellationToken ct)
    {
        ...
        return Assert.IsType<string>(await command.ExecuteScalarAsync(ct));
    }
    ...
    private sealed record St10bIncidentSnapshot(Guid Id, Guid ProbeId, string RuleKey, AvailabilityIncidentStatus Status, DateTimeOffset OpenedAt, DateTimeOffset? AcknowledgedAt, Guid? AcknowledgedBy, string? AcknowledgementComment, DateTimeOffset? ResolvedAt, Guid? ResolvedBy, string? ResolutionNote, int OccurrenceCount);
